...
# takes in argument of which MongoDB collection to use: 'basketball_2016_h2h'
...
# everything is updated back into 'basketball_2016_roto' collection
# takes in input of which MongoDB collection to use: 'basketball_2016_roto'
def roto_trifecta(collection):	
	...
	for this_category in categories:
		...
		for i in range(len(category_rank)):
			...
			current_team_h2h = category_rank[i]["h2h_rank"]
...
### now that all roto category points have been calculated, need to sum them
	...
	for each_team_dict in team_list:
		current_team_h2h = each_team_dict["h2h_rank"]
		...
		category_pull = {
			"category_points_FG_PCT" : 1,
			"category_points_FT_PCT" : 1,
			"category_points_THREEPM" : 1,
			"category_points_REB" : 1,
			"category_points_AST" : 1,
			"category_points_STL" : 1,
			"category_points_BLK" : 1,
			"category_points_TO" : 1,
			"category_points_PTS" : 1,
			"_id" : 0
			}
...
### follow same process as outlined before to rank teams, find ties, and distribute the proper number of roto trifecta points (10 -> 1)
	...
	for i in range(len(sorted_roto_rank)):
		...
		current_team_h2h = sorted_roto_rank[i]["h2h_rank"]
...
collection1 = db.basketball_2016_h2h
collection2 = db.basketball_2016_roto
